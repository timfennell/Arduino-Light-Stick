...
//When the user presses the Blue Button once the accelerometer controls the hue and saturation.
//When the user presses the Blue Button a second time the accelerometer no longer controls the hue and saturation and those settings freeze where they are.
//When the user presses the Red Button the light stick cycles through the modes (lighting patterns)
//When the user pressed the Green Button the Brightness cycles through different levels. (10% 20% 30% 40% 50% 60% 70%)
...
int STEPS = 2;           //Wider or narrower bands of color
...
uint8_t wavebright = 80;                                     // You can change the brightness of the waves/bars rolling across the screen.
...
#define maxsteps 8                                           // Case statement wouldn't allow a variable.
...
void loop() {
    switch (ledMode) {
       case 0: BRIGHTNESS = 100; Rainbow(); break;              //Rainbow -- Change STEPS and SPEED to modify
